...
| Chrome/Edge extension (`extension-chromium/`) | `extension-chromium/vX.Y.Z` | CI packages the store `.zip` via `extension-chromium/package.sh` and attaches it to a GitLab Release; both stores sign at upload. The package is assembled from the shared code in `extension/` plus this directory's own manifest, so it carries its own `version` in `extension-chromium/manifest.json` |
