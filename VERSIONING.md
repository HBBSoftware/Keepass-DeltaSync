...
| Android app (`android/`) | `android/vX.Y.Z` | Built & signed outside CI (F-Droid / manual). The app also carries its own `versionCode` + `versionName` in `android/app/build.gradle.kts` |
...
`android/app/build.gradle.kts`, build the signed APK, then tag for the record:
